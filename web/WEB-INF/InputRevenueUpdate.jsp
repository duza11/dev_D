...
<jsp:useBean id="rb" class="accountbook.RevenueBlock" scope="request" />
                ...
                <div class="panel-body">
                    <form class="form-vertical" action="?action=update_rev&block_id=<%=request.getParameter("block_id")%>" method="post">
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <input class="form-control" type="date" name="date" value="<%=new SimpleDateFormat("yyyy-MM-dd").format(rb.getDate())%>" pattern="\d{4}-(0?[1-9]|1[0-2])-\d{1,2}" title="yyyy-MM-ddの形式で日付を入力してください" placeholder="日付（yyyy-MM-dd）" required>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <input class="form-control" type="text" name="place" value="<%=rb.getPlace()%>" placeholder="店名" >
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <%                                boolean addFlag = true;
                                List<RevenueItem> riList = rb.getRevenueItemList();
                            ...
                            <div class="form-group form-block" <%="id=\"form_block[" + i + "]\""%>>
                                <div class="row">
                                    <div class="col-md-2 col-xs-3">
                                        <select class="form-control" <%="name=\"kind[" + i + "]\" id=\"kind[" + i + "]\""%> required>
                                            ...
                                            <option value="<%=e.getKey()%>"
                                                    <%
                                                        if (riList.get(i).getKindId() == e.getKey()) {
                                                    %>
                                                    <%="selected"%>
                                                    <%
                                                        }
                                    ...
                                    <div class="col-md-2 col-xs-3">
                                        <input class="form-control" type="text" <%="name=\"item_name[" + i + "]\" id=\"item_name[" + i + "]\""%> value="<%=riList.get(i).getItemName()%>" placeholder="名前">
                                    </div>
                                    <div class="col-md-2 col-xs-3">
                                        <input class="form-control" type="number" <%="name=\"price[" + i + "]\" id=\"price[" + i + "]\""%> value="<%=riList.get(i).getPrice()%>" min="0" placeholder="金額" required>
                                    </div>
                                    <div class="col-md-2 col-xs-2">
                                        <input class="form-control" type="number" <%="name=\"count[" + i + "]\" id=\"count[" + i + "]\""%> value="<%=riList.get(i).getCount()%>" min="1" placeholder="数" required>
                                    </div>
                                    <div class="ac-btn col-md-1 col-xs-1">
                                        ...
                                        %>
                                        <div class="add-btn update-add-btn" title="Add"><button class="add btn btn-default" type="button">+</button></div>
                                        <div class="close-btn update-close-btn" title="Close"><button class="btn btn-default" type="button">-</button></div>
                                        <%
                                            }
                                        %>
                                    </div>
                        ...
                        <button class="btn btn-default" id="clear" type="button">クリア</button>
                    </form>
                </div>
            </div>
        </div>
